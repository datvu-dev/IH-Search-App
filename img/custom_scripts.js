  ...
  var $antigenother = jQuery('.antigen_other_root_wrap');
   $antigenother.isotope({
    ...
    masonry: {
      gutter:17,
      // rowHeight:'.grid_spacer',
       columnWidth:'.grid_spacer_outer'
    }
  });

   var $antigenother = jQuery('.antibody_other_root_wrap');
   $antigenother.isotope({
    // options
...
function registerClickFunctions(){
  ...
  jQuery('.full_expand').click(function(e){
    ...
    if ($this.parents('.grid_wrap').find('.singleElement').length > 1 ) {
    ...
    else if ($this.parents('.grid_wrap').find('.singleElement').length == 1) {
...
function colorChangeAntibody() {
  ...
  jQuery('#search_input').removeClass('search-option-antigen').addClass('search-option-antibody');
}
...
function colorChangeAntigen() {
  ...
  jQuery('#search_input').removeClass('search-option-antibody').addClass('search-option-antigen');
}
...
function searchButtonClick(){
  ...
  jQuery('#submit_search').click(function(e){
    ...
    // searchWithOptions(search_text, search_option);
    resetGridForSearch();
    close_other_wrappers();
...
function resetGrid(){
  ...
  jQuery('.antigen_root_wrap').isotope({ sortBy: 'isbtno' });
  assignTopPositionSubWrapper();
  jQuery('.result').empty().css('display','none');
}
...
function hideNotFound(){
  ...
  resetGrid();
    ...
    assignTopPositionSubWrapper();
    jQuery('.root_wrap').isotope({ filter: '.found' });
...
function serachInComments(search_text) {
  search_text = search_text.toLowerCase();
  jQuery('.search').filter(function(index){
    if(jQuery(this).data('value').indexOf(search_text) != -1){
...
function filterSearchWithOption(search_text, option){
  ...
  if (option === "isbtno") {
    jQuery('.isbt').filter(function(index){
      if(String(jQuery(this).data('value')).indexOf(search_terms[0]) == 0){
        return true;
      }
    }).closest('.grid_wrap:not(.found)').addClass('found');    
    hideNotFound();
  ...
  if (option === "antibody") {
    search_text1 = search_text.replace('anti ', '');
    search_text1 = 'anti ' + search_text1 + " antibody";
    console.log(search_text1);
    jQuery('.blood_group_name').filter(function(index){
      if(String(jQuery(this).data('value')).indexOf(search_text1) == 0){
        return true;
      }
    }).closest('.grid_wrap:not(.found)').addClass('found');    
    hideNotFound();
  ...
  if (option === "antigen") {
    search_terms[0] = search_terms[0] + " antigen";
    console.log(search_terms[0]);
    jQuery('.blood_group_name').filter(function(index){
      if(String(jQuery(this).data('value')).indexOf(search_terms[0]) == 0){
        return true;
      }
